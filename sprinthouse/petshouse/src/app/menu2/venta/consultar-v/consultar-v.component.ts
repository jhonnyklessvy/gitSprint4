...
export class ConsultarVComponent implements OnInit {
  ...
  listar(): void {
    ...
    this.http.get("http://petshouse.jelastic.saveincloud.net/Venta/consul", { responseType: "json" })
      .subscribe((res: any) => {
        console.log(res);
        this.inc=res;
      });
  ...
  eliminar(x: any): void {
    this.http.delete("http://petshouse.jelastic.saveincloud.net/Venta/elim/" + x)
      .subscribe((res: any) => {
        console.log(res);
        this.listar();
